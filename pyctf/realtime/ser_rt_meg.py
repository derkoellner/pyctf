...
def do_rt_meg():
    ...
    # Become a server, launch the SER client.

    os.system("python3 serDaemon.py &")	# @@@ pass arguments, daemonize properly
    os.system("python3 dataDaemon.py &")	# @@@ pass arguments, daemonize properly
    sock = server(5555)
    sock2 = server(5556)
    ...
    for sample, packet in acq.packet_gen():
        ...
        if n >= NSAMP:
            ...
            # Apply the filter and send the result to the SER server.

            for c in range(M):
                h[:, c] = dofilt(x[:, c], filt)

            swritearray(sock, h.T)
            swritearray(sock2, h.T)
